  ...
  let a = setInterval(async function() {
    if (config["enable-proxies"] !== true || proxy !== undefined && config["enable-proxies"] == true) {
      ...
      puppeteer.launch(opt).then(async function(browser) {
        ...
        await page.goto("https://old.reddit.com/", {timeout: 30000});
        bar.increment(1, {
...
async function end(user, pass, eu, ep, b) {
  ...
  bar.stop();
  if (!fs.existsSync(`accounts/json`)) fs.mkdirSync(`accounts/json`);
  if (!fs.existsSync(`accounts/json/${user}.json`)) fs.writeFileSync(`accounts/json/${user}.json`, JSON.stringify({
    username: user,
    password: pass,
    email: {
      login: eu,
  ...
  if (!fs.existsSync(`accounts/csv`)) fs.mkdirSync(`accounts/csv`);
  if (!fs.existsSync(`accounts/csv/accounts.csv`)) fs.writeFileSync(`accounts/csv/accounts.csv`, convertoCSV({
    username: user,
    password: pass
  }));

  if (fs.existsSync(`accounts/csv/accounts.csv`)) fs.appendFileSync(`accounts/csv/accounts.csv`, convertoCSV({
    username: user,
    password: pass
  }));
